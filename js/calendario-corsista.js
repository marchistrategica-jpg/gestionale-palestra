...
function renderCalendar() {
  ...
  // Le mie prenotazioni confermate per slotId
  var myConfirmed = {};
    ...
    for (var di2=0; di2<6; di2++) {
      ...
      slotsHere.forEach(function(slot) {
        ...
        html += '<div class="corsista-slot ' + cls + '" data-slotid="' + slot.id + '">'
          ...
          + '<div class="slot-info">' + (istrName.split(' ')[0]||'—') + '</div>'
          + '<div class="slot-info">' + (isClosed?'Chiuso':postiLabel) + (isPrenotato?' ✓':'') + '</div>'
          + '</div>';
      });
  ...
  grid.querySelectorAll('.corsista-slot').forEach(function(el) {
    el.addEventListener('click', function() {
      ...
      if (!slot) return;
      openPopupSlot(slot, myConfirmed);
    });
  });
}

// ── Popup slot ────────────────────────────────────────────────
function openPopupSlot(slot, myConfirmed) {
  var course  = allCourses.find(function(c){return c.id===slot.courseId;});
  var room    = allRooms.find(function(r){return r.id===slot.roomId;});
  var istr    = allInstructors.find(function(i){return i.id===slot.instructorId;});
  var bks     = allBookings.filter(function(b){return b.slotId===slot.id&&b.status==='confirmed';}).length;
  var cap     = slot.maxCapacity || 5;
  var isFull  = bks >= cap;
  var isClosed= slot.isActive === false;
  var isPrenotato = myConfirmed[slot.id] !== undefined;
  ...
  var bodyHtml = ''
    ...
    + '<div class="popup-info-row"><span class="popup-label">Posti</span><span class="popup-val">' + bks + '/' + cap + '</span></div>';

  document.getElementById('modalSlotBody').innerHTML = bodyHtml;

  var footerHtml = '';
  if (isClosed) {
    footerHtml = '<span style="color:var(--slot-closed);font-size:.82rem;font-weight:700;">Sala non disponibile</span>';
  } else if (isPrenotato) {
  ...
  // Riaggiungi listener [data-close]
  document.querySelectorAll('[data-close]').forEach(function(btn) {
    btn.addEventListener('click', function() { closeModal(btn.dataset.close); });
  });
...
async function prenotaSlot(slotId) {
  ...
  var cap  = slot.maxCapacity || 5;
  if (bks >= cap)         { showToast('Lezione al completo!', 'error'); closeModal('modalSlot'); return; }

  var giaPrenotato = myBookings.find(function(b){return b.slotId===slotId&&b.status==='confirmed';});
  if (giaPrenotato)       { showToast('Hai già prenotato questa lezione.', 'info'); closeModal('modalSlot'); return; }
...
function renderMiePrenotazioni() {
  ...
  var istrMap   = {};
  allInstructors.forEach(function(i){istrMap[i.id]=i;});
  var slotMap   = {};
  allSlots.forEach(function(s){slotMap[s.id]=s;});
  ...
  if (!prossime.length && !passate.length) {
    html = '<div style="text-align:center;padding:40px;color:var(--text-muted);">'
      ...
      + '</svg><div style="font-weight:700;">Nessuna prenotazione</div>'
  ...
  if (prossime.length) {
    ...
    prossime.forEach(function(item) {
      ...
      var slotDate = new Date(item.slot.date);
      html += '<div class="prenotazione-card">'
        ...
        + '<div class="pren-details">' + formatDateLong(slotDate) + ' · ' + hour + ' · ' + ((sala&&sala.name)||'—') + ' · ' + ((istr&&istr.name)||'—') + '</div>'
        + '</div>'
        + '<button class="btn btn-danger" style="font-size:.75rem;padding:6px 12px;" data-bookid="' + item.b.id + '">Disdici</button>'
        + '</div>';
    });
  }
  ...
  if (passate.length) {
    ...
    passate.slice(0,10).forEach(function(item) {
      ...
      var hour  = String(item.slot.hour).padStart(2,'0') + ':00';
      var slotDate = new Date(item.slot.date);
      html += '<div class="prenotazione-card" style="opacity:.7;">'
        ...
        + '<div class="pren-info">'
        + '<div class="pren-corso">' + ((corso&&corso.name)||'Lezione') + '</div>'
        + '<div class="pren-details">' + formatDateLong(slotDate) + ' · ' + hour + ' · ' + ((sala&&sala.name)||'—') + '</div>'
        + '</div>'
  ...
  // Listener pulsanti disdici
  container.querySelectorAll('[data-bookid]').forEach(function(btn) {
    btn.addEventListener('click', function() {
      if (confirm('Vuoi disdire questa prenotazione?')) {
        disdiciSlot(btn.dataset.bookid);
      }
    });
  });
}
