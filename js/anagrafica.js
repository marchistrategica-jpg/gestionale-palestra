...
function renderTabella(soci) {
      ...
      <tr>
        ...
        <td>
          <div style="display:flex;gap:6px;flex-wrap:wrap;">
            ...
            <button class="btn btn-danger btn-icon btn-elimina" data-id="${s.id}" title="Elimina">
  ...
  tbody.querySelectorAll('.btn-elimina').forEach(el => {
    el.addEventListener('click', () => {
      const s = allSoci.find(x => x.id === el.dataset.id);
      const nome = s ? `${s.name||''} ${s.surname||''}`.trim() : 'questo socio';
      eliminaSocio(el.dataset.id, nome);
    });
  });
}
